...
const publicationPages = document.querySelectorAll('.publicationpage');

function moveDiv() {
    const checkedRadio = document.querySelector('.radio-group input[type="radio"]:checked + label');
    const radioRect = checkedRadio.getBoundingClientRect();
    const groupRect = radioGroup.getBoundingClientRect();
    const offsetLeft = radioRect.left - groupRect.left;

    radioDiv.style.left = `${offsetLeft}px`;
}

function showPage() {

    const checkedValue = document.querySelector('.radio-group input[type="radio"]:checked').value;
    var mubiaoid="publicationpage-page"+(2024-checkedValue+1);
    publicationPages.forEach((page, index) => {
